...
<body>
    ...
    <table width="100%" style="border-collapse: collapse; font-size: 11px;">
        ...
        <tr>
            ...
            <td style="border: 1px solid black; text-align: center; padding: 12px 0;">{{ $mataKuliah->semester }}</td>
        ...
        <tr>
            ...
            <td style="border: 1px solid black; font-weight: bold; text-align: center;" colspan="2">Koord. Prodi</td>
        </tr>
        <tr>
            <td style="border: 1px solid black; text-align: center; padding: 12px 0;"></td>
        ...
        <tr>
            <td style="border: 1px solid black; font-weight: bold; text-align: center;">Otorisasi</td>
            <td style="border: 1px solid black; font-weight: bold; text-align: center;" colspan="2">Ka Jurusan</td>
            <td style="border: 1px solid black; font-weight: bold; text-align: center;" colspan="2">Wakil Direktur Bidang Akademik</td>
        </tr>
        <tr>
            <td style="border: 1px solid black; text-align: center; padding: 12px 0;"></td>
            <td style="border: 1px solid black; text-align: center; padding: 12px 0;" colspan="2">
    ...
    <table width="100%" style="border: 1px solid black; border-collapse: collapse; font-size: 11px;">
        ...
        @else
            <tr>
                <td colspan="5" style="border: 1px solid black; text-align: center;">Tidak ada tujuan belajar.</td>
            </tr>
        @endif
    </table>
    ...
    <table width="100%" style="border-collapse: collapse; font-size: 11px;">
        <tbody>
            <tr>
                ...
                <th style="border: 1px solid black;">Modalitas, Bentuk, Strategi, dan Metode Pembelajaran (Media dan Sumber Belajar)</th>
                ...
                <th style="border: 1px solid black;">Total Bobot</th>
            </tr>
            <tr>
            ...
            @foreach ($mataKuliah['rpss'] as $rps)
                ...
                @else
                    <tr>
                        ...
                        <td style="border: 1px solid black; text-align: center;">{{ $rps['hasil_belajar'] }}</td>
                        @if (!empty($rps->instrumenPenilaians) && count($rps->instrumenPenilaians) > 0)
                            ...
                            <td style="border: 1px solid black; text-align: center;">
                                @foreach ($rps->instrumenPenilaians as $instrumen)
                                    @php
                                        $tujuan = collect($mataKuliah['tujuanBelajarRps'])->firstWhere('id', $instrumen->tujuan_belajar_id);
                                    @endphp
                                    {{ $tujuan['kode'] ?? '-' }}<br>
                                @endforeach
                            </td>
                            <td style="border: 1px solid black; text-align: center;">
                                @foreach ($rps->instrumenPenilaians as $instrumen)
                                    @php
                                        $cpl = collect($mataKuliah['cpls'])->firstWhere('id', $instrumen->cpl_id);
                                    @endphp
                                    {{ $cpl['kode'] ?? '-' }}<br>
                                @endforeach
                            </td>
                            <td style="border: 1px solid black; text-align: center;">
                                @foreach ($rps->instrumenPenilaians as $instrumen)
                                    {{ $instrumen->bobot_penilaian }}<br>
                                @endforeach
                            </td>
                        @else
                            <td colspan="4" style="border: 1px solid black; padding: 5px; text-align: center;">
                                Tidak ada instrumen penilaian
                            </td>
                        @endif
                        <td style="border: 1px solid black; padding: 5px; text-align: center;">
                             {{ $rps->instrumenPenilaians->sum('bobot_penilaian') }}
                        </td>
                    </tr> 
                @endif
            @endforeach
    ...
    <p style="font-size: 12px">
        Segala kecurangan (cheating, plagiat, copy-paste dan sejenisnya) tidak akan ditoleransi. Apabila mahasiswa
    ...
    <p style="font-size: 12px">
        Rencana Pembelajaran Semester (RPS) ini berlaku Tahun Akademik 2025/2026 dan seterusnya. RPS ini dievaluasi
